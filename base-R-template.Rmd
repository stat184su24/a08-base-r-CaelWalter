...
plotNormal <- function(repitions, mean, sd){
  data <- rnorm(repitions, mean = mean, sd = sd)
  hist(data)
}
...
```{r}
set.seed(184)
plotNormal <- function(repitions, mean, sd, col = "grey") {
  data <- rnorm(repitions, mean = mean, sd = sd)
  hist(data, main = paste("Histogram from a Normal(", mean, ", ", sd, ") Distribution", sep = ""), col = col)
}

plotNormal(1000, 0, 1, col = "blue")
